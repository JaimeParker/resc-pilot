...
#ifndef PLAN_ADMIN_K_GPEP_H
#define PLAN_ADMIN_K_GPEP_H
...
private:
    ...
    double coll_thresh_sq_ = sqrt(2) * collision_threshold_;
    double experience_gap_dist_ = 0.9;
    double res_ = 0.1;
...
public:
    ...
    std::vector<double> getKinematicPseudoRaycast(const Eigen::Vector3d & pos, const Eigen::Vector3d & vel,
                                                  const Eigen::Vector3d & ctrl_p);
    std::vector<double> getSurroundingSDF(const Eigen::Vector3d & pos);
    [[nodiscard]] Eigen::Vector2d getRelativePos(const Eigen::Vector3d & pos) const;
    ...
    [[nodiscard]] bool inline isInflateOccupied(const Eigen::Vector3d & pos) const;
};


bool inline KGPEP::isInflateOccupied(const Eigen::Vector3d &pos) const {
    return map_bridge_ptr_->getDistance(pos) < collision_threshold_ - 0.01;
}

#endif //PLAN_ADMIN_K_GPEP_H
