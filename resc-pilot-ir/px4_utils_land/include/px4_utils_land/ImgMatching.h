...
private:
    ...
    void updateOffsetWithFilter(const cv::Mat& frame, const cv::Point2f& centroid);
    ...
    // 红外灯光检测相关方法
    cv::Point2f detectByMaxBrightness(const cv::Mat& gray);
    cv::Point2f detectByBlobDetection(const cv::Mat& gray);
    cv::Point2f refineCenter(const cv::Mat& image, const cv::Point& rough_center, int window_size);
...
public:
    ...
    cv::Point2f detectIRLight(const cv::Mat& image);  // 自动检测红外灯光位置（公有方法）
